...
train_data_dir = '../data_base/trainL'
validation_data_dir = '../data_base/validationL'
...
model.save_weights('lines_weights_HSV.h5')
model.save('lines_model_HSV.h5')
...
model.load_weights('lines_weights_HSV.h5')
    ...
    for batch_num in range(2):
        ...
        for i in range(np.min([20,batch_size])):
            ...
            plt.title("Predicted: {0} // Truth: {1} // Actual score: {2}".format(
                int(predictions[i]>score_for_line), true_category[i], predictions[i][0] ))
            plt.show()
...
#        plt.imshow(np.divide(heatMapAdd, np.max(heatMapAdd)))
        ...
        figIM.show()
        figIM.savefig(('outImagesLines/'+imName[:-4]+'_lin.png'), dpi = 200)
...
#        heatMapSquares = np.divide(heatMapSquares, np.max(heatMapSquares))
        ...
        figHM.show()
        figHM.savefig(('outImagesLines/'+imName[:-4]+'_HM.png'), dpi = 200)
